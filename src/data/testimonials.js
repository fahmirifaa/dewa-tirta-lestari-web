export const testimonialsData = [
  {
    id: 1,
    name: "Budi Santoso",
    role: "Pemilik Rumah Makan Sederhana",
    avatar: "https://i.pravatar.cc/150?img=12",
    rating: 5,
    message:
      "Sudah 2 tahun berlangganan air dari TIRTA KENCANA ABADI untuk kebutuhan dapur rumah makan saya. Kualitas air selalu bersih dan pengiriman selalu tepat waktu.",
  },
  {
    id: 2,
    name: "Siti Nurhaliza",
    role: "Ibu Rumah Tangga",
    avatar: "https://i.pravatar.cc/150?img=45",
    rating: 5,
    message:
      "Pelayanannya ramah dan responsif via WhatsApp. Air yang dikirim juga jernih dan tidak berbau, aman untuk kebutuhan keluarga sehari-hari.",
  },
  {
    id: 3,
    name: "Ahmad Hidayat",
    role: "Manajer Operasional PT Sinar Jaya",
    avatar: "https://i.pravatar.cc/150?img=33",
    rating: 4,
    message:
      "Untuk kebutuhan air industri pabrik kami, TIRTA KENCANA ABADI selalu bisa diandalkan. Harga juga transparan sesuai kesepakatan awal.",
  },
  {
    id: 4,
    name: "Rina Wijaya",
    role: "Pengelola Hotel Melati Indah",
    avatar: "https://i.pravatar.cc/150?img=28",
    rating: 5,
    message:
      "Kebutuhan air hotel kami cukup besar, dan mereka selalu siap memenuhi permintaan mendadak sekalipun. Sangat membantu operasional hotel.",
  },
  {
    id: 5,
    name: "Dedi Kurniawan",
    role: "Ketua DKM Masjid Al-Ikhlas",
    avatar: "https://i.pravatar.cc/150?img=51",
    rating: 5,
    message:
      "Alhamdulillah kebutuhan air wudhu jamaah masjid kami selalu tercukupi berkat kerjasama dengan TIRTA KENCANA ABADI. Semoga terus amanah.",
  },
];